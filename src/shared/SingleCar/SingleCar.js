    ...
    <div className="md:mx-8 mx-2">
      <div className="p-4 px-6 shadow-md rounded bottom-1">
        <div className="relative">
          ...
          <span className={`p-1 text-lg text-white font-semibold rounded px-2 absolute top-3 right-2 ${Availability === 'available' ? 'bg-green-600' : 'bg-red-600' }`}>{Availability}</span>

         {
            advertised &&  <span className="p-1 text-lg text-slate-600 font-semibold rounded px-2 absolute top-3 left-2 bg-yellow-400">{advertised && 'Featured' }</span>
         }
        ...
        <div className="mt-4 flex md:flex-row flex-col justify-between gap-4">
            <span className="text-xl font-semibold flex items-center  gap-1"><BsCalendarDate/> {pubDate}</span>
            <span className="text-xl font-semibold text-slate-800">Years Of Used :{uses}</span>
            <span className="text-xl font-semibold text-slate-800 flex items-center  gap-1"><BiUserCircle/>{seller}
            {
        ...
        <p className="text-lg font-semibold mt-4">Seller Contact : {sellerNo}</p>
        <button  className="btn bg-yellow-400 mt-6 hover:text-white border-0 text-lg text-slate-900">Book Now</button>
      </div>
    </div>
  );
};
